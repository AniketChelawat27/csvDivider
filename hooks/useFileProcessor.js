...
export function useFileProcessor() {
  ...
  const processFiles = async (files, divideInto) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const processData = await CSVProcessingService.processFile(
          file,
          divideInto,
          uploadedFileUrl
        );
        ...
        const splitFileUploads = await CSVProcessingService.uploadSplitFiles(
          processData.filesCreated
        );
  ...
  const processMultipleFiles = async (files, title = "") => {
    ...
    try {
      ...
      console.log("✅ Multiple files uploaded successfully:", uploadResult);
    } catch (err) {
